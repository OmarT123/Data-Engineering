...
def extract_clean(fileName):
    ...
    cleaned_df, _ = clean(df)
    print("Data Cleaned")
    df.to_csv("./data/fintech_clean.csv")
...
def transform(fileName):
    ...
    df.to_csv("./data/fintech_transformed.csv")
...
def load_to_db(fileName):
    ...
    df = read_from_csv(fileName)
    engine = create_engine('postgresql://root:root@pgdatabase:5432/fintech_data')

    if(engine.connect()):
        print('Connected to Database')
        try:
            print('Writing cleaned dataset to database')
            df.to_sql('fintech_db_MET_P01_52_11870', con=engine, if_exists='replace')
